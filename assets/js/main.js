...
$(function() {
	...
	{
		wit.each(function(index,item){
			item = $(item);
			currentQuestion = 0;
			var content = item.find(".content");
			var itemWrapper = item.find(".items-wrapper");
			var items = item.find(".item");
			var witForm = item.find("form");;
			var indicators = item.find(".indicators a");
			var contentWidth = content.width();

			$(witForm).submit(function(e){
				e.preventDefault();
				//$(witForm).ajaxForm({url: $(witForm).attr("action"), type: 'post'});
				var serialized = $(witForm).serialize();
				serialized += "&index="+currentQuestion;
				$.post(witForm.attr("action"), serialized );
				item.fadeOut();
			})
			itemWrapper.css({"width":items.length*contentWidth})
			items.each(function(index,item){
				item = $(item);
				item.css({"width":contentWidth});
				item.find(".btn-group > button.btn").on("click", function(e){
					item.addClass("passed");
					item.find("button").removeClass("btn-success");
					$(e.target).addClass("btn-success");
					$.post(witForm.attr("action"), { index:currentQuestion,value:$(e.target).text() } );
					this.showQuestion(currentQuestion+1);
				}.bind(this));
			}.bind(this));

			item.find(".next").click(function(e){
				e.preventDefault();
				this.showQuestion(currentQuestion+1);
			}.bind(this));
			...
			this.showQuestion = function(index)
			{
				if(index > -1 && index<items.length)
				{
					...
					itemWrapper.animate({"right":-1*contentWidth*index+margin});
					content.animate({"height":currentItem.height()});
					currentQuestion = index;
					item.find(".indicators .selected").removeClass("selected");
					$(indicators[index]).addClass("selected");
				}
			}

			indicators.each(function(index,currentItem){
...
function isotopInitialization(isotopElement)
{
    ...
    gridsterElements.popover({"trigger":"manual","placement":"top","html":true});
    gridsterElements.bind("mouseenter",function(e){
    	var element = $(e.currentTarget);
    	if(element.offset().top-$(window).scrollTop() < 160)
    	{
    		element.popover('destroy');
    		element.popover({"trigger":"manual","placement":"bottom","html":true});
    	}
    	else
    	{
    		element.popover('destroy');
    		element.popover({"trigger":"manual","placement":"top","html":true});
    	}
    	element.popover("show");
    });

    gridsterElements.bind("mouseleave",function(e){
    	var element = $(e.currentTarget);
